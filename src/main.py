...
4. Ctrl+C 정상 종료 지원 (signal handler)
"""
...
async def lifespan(app: FastAPI):
    ...
    if API_ROUTERS_LOADED:
        ...
        try:
            ...
            # 각 더미 수집기의 ensure_tables() 직접 호출
            dummy_modbus_collector.ensure_tables()
            dummy_env_collector.ensure_tables()
        ...
        def verify_data_collection():
            ...
            try:
                ...
                with get_cursor() as cur:
                    # 분리 테이블 데이터 개수 확인
                    modbus_count = 0
                    for device_id in [11, 12, 13, 14, 15]:
                        try:
                            cur.execute(f"SELECT COUNT(*) FROM modbus_data_{device_id}")
                            modbus_count += cur.fetchone()[0]
                        except Exception:
                            pass  # 테이블 없으면 건너뛰기
                    
                    env_count = 0
                    for device_id in [21, 22, 23]:
                        try:
                            cur.execute(f"SELECT COUNT(*) FROM env_data_{device_id}")
...
if API_ROUTERS_LOADED:
    ...
    # 각 센서별 API 라우터 등록
    app.include_router(modbus_router.router, tags=["modbus"], prefix="/api")    # 전력 데이터 API
    app.include_router(env_router.router, tags=["env"], prefix="/api")          # 환경 데이터 API  
    app.include_router(solar_router.router, tags=["solar"], prefix="/api")      # 태양광 데이터 API
...
def run_server():
    ...
    Features:
    - Ctrl+C (SIGINT) 정상 종료 지원
    - daemon=True 백그라운드 쓰레드 자동 종료
    """
    ...
    def signal_handler(sig, frame):
        ...
        - daemon=True로 실행된 백그라운드 쓰레드들은 자동 종료됨
        - 깔끔한 종료 메시지 출력
        """
        print("\n\n" + "="*60)
        print("🛑 Ctrl+C 감지! 서버 종료 중...")
        print("="*60)
        print("📊 백그라운드 수집기 쓰레드들 자동 종료 (daemon=True)")
        print("✅ 안전하게 종료되었습니다")
        print("="*60 + "\n")
        sys.exit(0)
