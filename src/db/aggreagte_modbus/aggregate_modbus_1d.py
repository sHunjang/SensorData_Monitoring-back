"""
Modbus 1일 집계 스크립트 (Device 11-15)

목적:
  - agg_modbus_11~15_1h (1시간 집계) → agg_modbus_11~15_1d (1일 집계)
...
log = logging.getLogger("agg_modbus_1d")
...
def get_aggregate_sql(device_id: int, start_time: datetime, end_time: datetime) -> str:
    """1일 집계 SQL 생성"""
    return f"""
    INSERT INTO agg_modbus_{device_id}_1d (
        bucket,
    ...
    FROM agg_modbus_{device_id}_1h
    WHERE bucket >= %s
      AND bucket < %s
    GROUP BY bucket_1d
...
def aggregate_all(days: int = 7):
    ...
    log.info("=" * 70)
    log.info("Modbus 1-Day Aggregation")
    log.info("=" * 70)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Modbus 1-Day Aggregation")
    parser.add_argument('--days', type=int, default=7, help='Days to aggregate')
