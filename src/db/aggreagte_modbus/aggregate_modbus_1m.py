"""
Modbus 1분 집계 스크립트 (Device 11-15)

실행:
  python -m src.db.aggregate_modbus.aggregate_modbus_1m --hours 168
"""
...
log = logging.getLogger("agg_modbus_1m")
...
def get_aggregate_sql(device_id: int, start_time: datetime, end_time: datetime) -> str:
    """1분 집계 SQL 생성"""
    return f"""
    INSERT INTO agg_modbus_{device_id}_1m (
        bucket,
...
def aggregate_all(hours: int = 24):
    ...
    log.info("=" * 70)
    log.info("Modbus 1-Minute Aggregation")
    log.info("=" * 70)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Modbus 1-Minute Aggregation")
