"""
Modbus 1개월 집계 스크립트 (Device 11-15)

목적:
  - agg_modbus_11~15_1d (1일 집계) → agg_modbus_11~15_1mo (1개월 집계)
...
log = logging.getLogger("agg_modbus_1mo")
...
def get_aggregate_sql(device_id: int, start_time: datetime, end_time: datetime) -> str:
    """1개월 집계 SQL 생성"""
    return f"""
    INSERT INTO agg_modbus_{device_id}_1mo (
        bucket,
...
def aggregate_all(months: int = 6):
    ...
    start_time = end_time - timedelta(days=months * 30)  # 근사값
    
    log.info("=" * 70)
    log.info("Modbus 1-Month Aggregation")
    log.info("=" * 70)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Modbus 1-Month Aggregation")
    parser.add_argument('--months', type=int, default=6, help='Months to aggregate')
