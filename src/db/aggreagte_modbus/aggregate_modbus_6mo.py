"""
Modbus 6개월 집계 스크립트 (Device 11-15)

목적:
  - agg_modbus_11~15_1mo (1개월 집계) → agg_modbus_11~15_6mo (6개월 집계)
...
log = logging.getLogger("agg_modbus_6mo")
...
def get_aggregate_sql(device_id: int, start_time: datetime, end_time: datetime) -> str:
    """6개월 집계 SQL 생성"""
    return f"""
    INSERT INTO agg_modbus_{device_id}_6mo (
        bucket,
    ...
    SELECT
        time_bucket('6 months', bucket) AS bucket_6mo,
    ...
    FROM agg_modbus_{device_id}_1mo
    WHERE bucket >= %s
      AND bucket < %s
    GROUP BY bucket_6mo
...
def aggregate_all(years: int = 2):
    end_time = datetime.now(timezone.utc)
    start_time = end_time - timedelta(days=years * 365)
    
    log.info("=" * 70)
    log.info("Modbus 6-Month Aggregation")
    log.info("=" * 70)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Modbus 6-Month Aggregation")
    parser.add_argument('--years', type=int, default=2, help='Years to aggregate')
    args = parser.parse_args()
    aggregate_all(years=args.years)
