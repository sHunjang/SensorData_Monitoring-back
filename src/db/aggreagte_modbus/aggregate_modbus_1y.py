"""
Modbus 1년 집계 스크립트 (Device 11-15)

목적:
  - agg_modbus_11~15_6mo (6개월 집계) → agg_modbus_11~15_1y (1년 집계)
...
log = logging.getLogger("agg_modbus_1y")
...
def get_aggregate_sql(device_id: int, start_time: datetime, end_time: datetime) -> str:
    """1년 집계 SQL 생성"""
    return f"""
    INSERT INTO agg_modbus_{device_id}_1y (
        bucket,
    ...
    SELECT
        time_bucket('1 year', bucket) AS bucket_1y,
    ...
    FROM agg_modbus_{device_id}_6mo
    WHERE bucket >= %s
      AND bucket < %s
    GROUP BY bucket_1y
...
def aggregate_all(years: int = 5):
    """모든 장치의 1년 집계 실행"""
    end_time = datetime.now(timezone.utc)
    start_time = end_time - timedelta(days=years * 365)
    
    log.info("=" * 70)
    log.info("Modbus 1-Year Aggregation")
    log.info("=" * 70)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Modbus 1-Year Aggregation")
    parser.add_argument('--years', type=int, default=5, help='Years to aggregate')
