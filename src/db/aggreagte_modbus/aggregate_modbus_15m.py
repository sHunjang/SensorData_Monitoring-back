"""
Modbus 15분 집계 스크립트 (Device 11-15)
...
log = logging.getLogger("agg_modbus_15m")
...
# ============================================================================
# 집계 SQL (수정: GROUP BY를 time_bucket 결과에 직접 적용)
# ============================================================================

def get_aggregate_sql(device_id: int, start_time: datetime, end_time: datetime) -> str:
    """15분 집계 SQL 생성"""
    return f"""
    INSERT INTO agg_modbus_{device_id}_15m (
        bucket,
    ...
    SELECT
        time_bucket('15 minutes', bucket) AS bucket_15m,
    ...
    FROM agg_modbus_{device_id}_1m
    WHERE bucket >= %s
      AND bucket < %s
    GROUP BY bucket_15m
...
def aggregate_all(hours: int = 24):
    ...
    log.info("=" * 70)
    log.info("Modbus 15-Minute Aggregation")
    log.info("=" * 70)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Modbus 15-Minute Aggregation")
