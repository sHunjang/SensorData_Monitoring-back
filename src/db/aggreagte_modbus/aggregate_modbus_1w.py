"""
Modbus 1주 집계 스크립트 (Device 11-15)

목적:
  - agg_modbus_11~15_1d (1일 집계) → agg_modbus_11~15_1w (1주 집계)
...
log = logging.getLogger("agg_modbus_1w")
...
def get_aggregate_sql(device_id: int, start_time: datetime, end_time: datetime) -> str:
    """1주 집계 SQL 생성"""
    return f"""
    INSERT INTO agg_modbus_{device_id}_1w (
        bucket,
    ...
    SELECT
        time_bucket('7 days', bucket) AS bucket_1w,
    ...
    FROM agg_modbus_{device_id}_1d
    WHERE bucket >= %s
      AND bucket < %s
    GROUP BY bucket_1w
...
def aggregate_all(weeks: int = 4):
    """모든 장치의 1주 집계 실행"""
    end_time = datetime.now(timezone.utc)
    start_time = end_time - timedelta(weeks=weeks)
    
    log.info("=" * 70)
    log.info("Modbus 1-Week Aggregation")
    log.info("=" * 70)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Modbus 1-Week Aggregation")
    parser.add_argument('--weeks', type=int, default=4, help='Weeks to aggregate')
    args = parser.parse_args()
    aggregate_all(weeks=args.weeks)
